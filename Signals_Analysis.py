...
def Saturation(tr,plot):
    ...
    for i in xrange(len(indexselected)-1) : 
        if tr.data[indexselected[i+1]] == tr.data[indexselected[i]]:
            Lsat.append(True)
        elif tr.data[indexselected[i+1]]>=0.997 * tr.data[indexselected[i]] and tr.data[indexselected[i+1]]<=(1+0.003)*tr.data[indexselected[i]] and abs(indexselected[i+1]-indexselected[i])<3:
            Lsat.append(True)
        else:
            Lsat.append(False)
